...
def get_initial_data():
    ...
    conn.close()
    return jsonify({'usuarios': [dict(row) for row in usuarios],'ubicaciones': [dict(row) for row in ubicaciones],'auxiliares': [dict(row) for row in auxiliares]})
...
def handle_prestamos():
    ...
    if request.method == 'POST':
        data = request.json
        if not all(k in data and data[k] for k in ['fecha', 'identificacion', 'nombre', 'ubicacion', 'hora_inicio', 'prestado_por']):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (data['fecha'], data['identificacion'], data['nombre'], data.get('area'),data.get('pc', False), data.get('pc_numero'), data.get('pc_pertenece'),data.get('kit', False), data.get('aire', False),data.get('cabinas', False), data.get('consola', False), data.get('vbeam', False),data['ubicacion'], data.get('edificio'), data['hora_inicio'], data['prestado_por'],data.get('observaciones')))
        conn.commit()
        conn.close()
...
def devolver_prestamo(prestamo_id):
    ...
    if prestamo:
        try:
            ...
            horas_utilizacion = None
    conn.execute("UPDATE prestamos SET hora_entrega = ?, recibido_por = ?, horas_utilizacion = ? WHERE id = ?", (datetime.now().strftime('%H:%M:%S'), recibido_por, horas_utilizacion, prestamo_id))
    conn.commit()
    conn.close()
...
def export_pdf():
    ...
    if not data:
        ...
        style = TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#34495e')),('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),('ALIGN', (0, 0), (-1, -1), 'CENTER'),('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),('FONTSIZE', (0, 0), (-1, -1), 6),('BOTTOMPADDING', (0, 0), (-1, 0), 10),('BACKGROUND', (0, 1), (-1, -1), colors.HexColor('#ecf0f1')),('GRID', (0, 0), (-1, -1), 1, colors.black)])
