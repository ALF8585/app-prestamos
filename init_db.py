...
def inicializar_bd():
    ...
    if not os.path.exists(excel_path):
        print(f"Error: No se encontró el archivo '{excel_path}'. Asegúrate de que está en la misma carpeta.")
        return

    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()

    # --- Tabla Principal de Préstamos (MODIFICADA) ---
    ...
    CREATE TABLE prestamos (
        ...
        pc BOOLEAN,
        pc_numero INTEGER,
        pc_pertenece TEXT, -- <<< NUEVA COLUMNA
        kit BOOLEAN,
        aire BOOLEAN,
        cabinas BOOLEAN,
        consola BOOLEAN,
    ...
    df_loc = df_loc.rename(columns={'SALONES Y OFICINA': 'nombre', 'EDIFICIO': 'edificio'})
    ...
    df_aux = df_aux.rename(columns={'AUXILIAR': 'nombre'})
    ...
    df_equipos = df_equipos.rename(columns={'EQUIPOS': 'id', 'PERTENECE': 'pertenece'})
    ...
    print("Base de datos inicializada correctamente. Los IDs duplicados han sido omitidos.")
    conn.commit()
    conn.close()
